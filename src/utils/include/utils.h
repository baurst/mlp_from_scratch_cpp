...
template <class T>
Mat2D<T>::Mat2D(size_t num_rows, size_t num_cols)
...
template <class T> Mat2D<T> Mat2D<T>::add(const Mat2D<T> &other) const {
  ...
  if (other.get_num_rows() == get_num_rows() &&
    ...
    for (size_t row_idx = 0;
         row_idx < std::max(other.get_num_rows(), get_num_rows()); ++row_idx) {
      for (size_t col_idx = 0;
           col_idx < std::max(other.get_num_cols(), get_num_cols());
           ++col_idx) {
        result(row_idx, col_idx) =
            this->operator()(std::max(get_num_rows(), row_idx),
                             std::max(get_num_cols(), col_idx)) +
            other(std::max(other.get_num_rows(), row_idx),
                  std::max(other.get_num_cols(), col_idx));
      }
    }
  }
  return result;
